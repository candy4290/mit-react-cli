 ...
 */
export function parseData(obj, key, value, isNum = false) {
...
export function getProvinceAndCityData(data) {
  const result = [];
  Object.keys(data['86']).forEach(item => {
    const temp = { label: data['86'][item], value: item, children: [] };
...
/* 数字加千分位-传入数字或字符串（数字） */
export function NumberToLocal(val) {
...
/* 将秒转为 x分钟y秒 */
export function transformSeconds(nums) {
 ...
 */
export function setUrlParams(url = throwIfMissing(), params = {}, isCoverOldParam = true) {
 ...
 */
export function parseUrlParams(url = throwIfMissing()) {
